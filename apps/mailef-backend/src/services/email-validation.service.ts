...
const MX_CACHE_TTL_MS = Math.max(
  Number(process.env.EMAIL_VALIDATION_MX_CACHE_TTL_MS) || 3_600_000, // 1 hour default
...
export async function hasMxRecord(domain: string): Promise<boolean> {
  ...
  try {
    ...
    if (!hasValidMx) {
      logger.debug("No valid MX records found for domain", { domain: normalizedDomain, mxRecords });
    }
    ...
    logger.warn("DNS MX lookup failed unexpectedly", {
 ...
 */
export async function batchValidateMxRecords(emails: string[]): Promise<BatchMxResult> {
  const valid: string[] = [];
...
export async function validateRecipientEmail(email: string): Promise<ValidationResult> {
  // 1. Syntax check
