...
exports.resetPassword = exports.forgotPassword = exports.getMyMailSettings = exports.getMyEmailActivity = exports.getMyEmailAllocationHistory = exports.getMyEmailAllocation = exports.getMe = exports.login = exports.register = void 0;
...
const signToken = (userId, role) => jsonwebtoken_1.default.sign({ userId, role }, process.env.JWT_SECRET, {
...
exports.getMe = getMe;
const getMyEmailAllocation = async (req, res) => {
    try {
        if (!req.userId) {
            res.status(401).json({ message: "Unauthorized" });
            return;
        }
        const summary = await (0, email_allocation_service_1.getEmailAllocationSummary)(req.userId);
        res.json(summary);
    }
    catch (error) {
...
exports.getMyEmailAllocation = getMyEmailAllocation;
const getMyEmailAllocationHistory = async (req, res) => {
    try {
        if (!req.userId) {
            res.status(401).json({ message: "Unauthorized" });
            return;
        ...
        const history = await (0, email_allocation_service_1.getUserEmailAllocationHistory)(req.userId, page, limit);
        res.json(history);
    }
    catch (error) {
...
exports.getMyEmailAllocationHistory = getMyEmailAllocationHistory;
const getMyEmailActivity = async (req, res) => {
    try {
        if (!req.userId) {
            res.status(401).json({ message: "Unauthorized" });
            return;
        ...
        const logs = await AuditLog_model_1.default.find(filter)
            .sort({ createdAt: -1 })
            .skip(skip)
            .limit(limit)
            .lean();
        const total = await AuditLog_model_1.default.countDocuments(filter);
...
exports.getMyEmailActivity = getMyEmailActivity;
const getMyMailSettings = async (req, res) => {
    try {
        if (!req.userId) {
            res.status(401).json({ message: "Unauthorized" });
            return;
        ...
        const [mailSettings, sendingDomains] = await Promise.all([
            (0, platform_settings_service_1.getPlatformMailSettings)(),
            (0, sending_domain_service_1.getAvailableSendingDomainNames)(),
        ]);
        res.json({
            mailSettings: {
...
exports.getMyMailSettings = getMyMailSettings;
