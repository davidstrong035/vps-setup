...
async function main() {
    ...
    if (!campaignId || !mongoose_1.default.Types.ObjectId.isValid(campaignId)) {
    ...
    const queue = new bullmq_1.Queue("mail-queue", { connection: redis });
    const db = mongoose_1.default.connection.db;
    const campaignObjId = new mongoose_1.default.Types.ObjectId(campaignId);
    // ── 1. Check campaign exists ───────────────────────────────────────────────
    const campaign = await db.collection("campaigns").findOne({ _id: campaignObjId }, { projection: { status: 1, stats: 1, userId: 1 } });
    if (!campaign) {
    ...
    const resetResult = await db.collection("campaignrecipients").updateMany({ campaignId: campaignObjId, status: { $in: ["failed", "queued"] } }, { $set: { status: "pending", lastError: null, retryCount: 0 } });
    ...
    await db.collection("campaigns").updateOne({ _id: campaignObjId }, { $set: { "stats.sent": sentCount, "stats.failed": 0, status: "sending", pauseReason: null } });
...
//# sourceMappingURL=requeue-failed-campaign.js.map